...
这不是个例。我翻了一下和同行们的聊天记录，过去半年有几个团队都在做类似的事：把项目从 CocoaPods 迁到 Swift Package Manager（SPM）。理由也差不多：CocoaPods 已经进入维护模式，trunk 有了只读时间表；新库越来越多地把 SPM 放在前面，Xcode 现在用 SPM 也比前几年省事。
...
CocoaPods 不是马上不能用，也不是明天就会把你的构建打爆。准确说，它已经进入维护模式。
...
> CocoaPods 没死。存量项目不用慌，但新项目还默认选它，就有点说不过去了。
...
现在的问题不是 CocoaPods 能不能装，而是很多库的文档、示例和排查思路都先按 SPM 写。继续用 CocoaPods 也能跑，只是经常要多绕一圈。

### 3. Xcode 里的 SPM 已经能用了
...
CocoaPods 这边，每次 Xcode 大版本更新，社区里总会冒出一波「pod install 卡住」「Build Phases 跑飞」之类的问题。原因不神秘：CocoaPods 通过 `Pods.xcodeproj`、xcconfig 和脚本阶段接入工程，和 Xcode 原生依赖模型不是一条线。Xcode 改一点构建细节，它可能就要跟着补。
...
Xcode 工程可以同时挂 SPM 依赖和 CocoaPods 依赖。能切的先切掉，剩下的慢慢来，不用追求一次性完美。
...
Flutter 这里要分场景。普通 Flutter App、Flutter module add-to-app、Flutter plugin 作者，三种场景都叫「用 SPM」，但你要维护的东西不一样。
...
如果你维护的是 Flutter 插件，情况又不一样。插件现在最好同时支持 CocoaPods 和 SPM，直到 Flutter 官方明确结束 CocoaPods 兼容期。也就是说，`podspec` 暂时不能删；同时给 iOS / macOS 插件补上 `Package.swift` 和 Swift package 目录结构。新项目走 SPM，老项目继续走 CocoaPods。
...
下面这些问题，迁移时很容易遇到。有些不一定难，但很耗排查时间。
...
迁移过程本身不算难，难的是开始动那份已经跑了很多年的 `Podfile`。它可能没人敢碰，也可能一改就牵出一串历史问题。

CocoaPods 还能用，但它已经进入维护模式；SPM 是 Swift 生态的官方方案。这事儿早晚要做。趁依赖还没失控时一点点迁，比哪天被迫迁要轻松得多。

如果你的项目正卡在迁移第一步，比如某个商业 SDK 没 SPM 支持，或者 CI 缓存一直搞不定，也可以留言。这个话题纸面上很清楚，落到老项目里，经常要一个依赖一个依赖地拆。
