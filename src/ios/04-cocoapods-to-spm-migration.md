...
> 记录一次从 Podfile 迁到 Package.swift 的过程：为什么要迁、哪些依赖先迁，以及 Flutter 项目怎么处理。

---

## 前言

最近团队里有人问了一句「我们的 Podfile 是不是该清理一下了」，我才发现：距离上次认真整理 `Podfile.lock`，已经过去了一年多。

这不是个例。我翻了一下和同行们的聊天记录，过去半年有几个团队都在做同一件事：把项目从 CocoaPods 迁到 Swift Package Manager（SPM）。原因也差不多：CocoaPods 已经进入维护模式，trunk 有了只读时间表；新库越来越多地把 SPM 放在前面，Xcode 对 SPM 的支持也比前几年顺手很多。

我后来花了两周，把项目里的依赖分批迁过去。这里把过程记下来，主要回答几个问题：

1. CocoaPods 现在到底是什么状态；
2. SPM 是什么，它的「标准」长什么样；
3. 真实项目怎么迁，哪些地方容易翻车。

如果你手上还有一份几年没动过的 `Podfile`，这篇应该能省一点排查时间。

---

## 一、CocoaPods 现状：还能用，但别再当默认选项

### 1. 官方已经把它放进维护模式

CocoaPods 不是马上不能用，也不是明天就会把你的构建打爆。更准确的说法是：它已经进入维护模式。

CocoaPods 官方博客在 2024 年 8 月写得很直接：CocoaPods 仍会维持运行，但项目已经处于 maintenance mode。随后 2024 年 11 月又公布了 trunk 只读计划：两年后把 CocoaPods trunk 切成只读，不再接受新的 Podspec 和版本发布。按官方计划，日期是 2026 年 12 月 2 日。

这件事对存量项目的影响分两层：

- 已有依赖不会立刻失效。Specs repo、CDN、GitHub 上已有的版本仍能继续被解析。
- 新版本发布会受影响。如果某个库只通过 CocoaPods trunk 分发，trunk 只读后就不能再通过它发布新版本。
- 私有 Specs 不受 trunk 只读直接影响，但仓库、权限、CI 和安全策略都要团队自己维护。

> CocoaPods 没死。存量项目不用恐慌，但新项目还默认选它，就有点说不过去了。
...
现在的问题不是 CocoaPods 能不能装，而是很多库的文档、示例和问题排查都先按 SPM 写。你继续用 CocoaPods，也能跑，但经常要多绕一圈。

### 3. Xcode 里的 SPM 已经够用了

从 Xcode 11 开始，Apple 就把 Swift Package 集成进了 Xcode。现在日常用下来，几个地方已经比较实用：
...
CocoaPods 这边，每次 Xcode 大版本更新，社区里总会冒出一波「pod install 卡住」「Build Phases 跑飞」之类的问题。原因不神秘：CocoaPods 通过 `Pods.xcodeproj`、xcconfig 和脚本阶段接入工程，和 Xcode 原生依赖模型不是一条线。Xcode 改一点构建细节，它就可能要跟着补。
...
它不是第三方工具，也不是某个社区项目。对 Swift 项目来说，这就是官方路线。
...
- 多个本地 package 一起开发时，不用再把依赖关系塞进 Podfile 里绕来绕去。
...
这个阶段我用得最多的是 Swift Package Index。它能快速看出一个库有没有 SPM 支持、支持到哪个平台、release 情况怎么样。

### 2. 步骤二：处理无法迁移的依赖

现实一点看，不是每个项目都能一次性迁干净。常见的卡点有这些：
...
Xcode 工程可以同时挂 SPM 依赖和 CocoaPods 依赖。先把能切的切掉，剩下的慢慢来，不要追求一次性完美。
...
如果项目准备顺手做模块化，可以先建一个本地 Package，把已经稳定的业务模块或基础模块迁进去，再在 manifest 里声明外部依赖：
...
这种写法的好处是边界清楚。App 工程先只依赖这个本地 package 的 product，package 内部再逐步接入三方依赖。

如果暂时不做模块化，只是把 App 里的第三方库从 Podfile 换成 SPM，就没必要强行建一个「空壳依赖包」。直接在 Xcode、XcodeGen 或 Tuist 里把 package product 加到 App target 上，通常更直观。
...
### 6. Flutter App / Module / Plugin 怎么处理

Flutter 这里不能一概而论。普通 Flutter App、Flutter module add-to-app、Flutter plugin 作者，三种场景都叫「用 SPM」，但你要维护的东西不一样。

![Flutter App、Module 和 Plugin 使用 SPM 的三种路径](../assets/inline-ios-04-flutter-spm-paths.png)

#### 6.1 普通 Flutter App：交给 Flutter 生成

如果项目是普通 Flutter App，不要手写 `Package.swift` 去接 Flutter 插件。Flutter 工具链会在 iOS 构建阶段生成并维护 `FlutterGeneratedPluginSwiftPackage`，用它把支持 SPM 的 Flutter 插件接进 Xcode 工程。
...
这里要小心：Flutter 会对还不支持 SPM 的插件回退到 CocoaPods。所以 Flutter 项目不要一上来就删 `Podfile`、`Podfile.lock` 和 `Pods/`。先执行构建，确认所有插件都能通过 SPM 接入；如果还有插件只能走 CocoaPods，就继续保留混合模式。
...
如果你维护的是 Flutter 插件，情况又不一样。插件现在最好同时支持 CocoaPods 和 SPM，直到 Flutter 官方明确结束 CocoaPods 兼容期。也就是说，`podspec` 暂时不能删；同时要给 iOS / macOS 插件补上 `Package.swift` 和标准 Swift package 目录结构。新项目走 SPM，老项目还能继续走 CocoaPods。
...
这一步别省。只验证 SPM 不够，因为还有没升级到 Flutter 3.44、或者主动关闭 SwiftPM 的项目会继续走 CocoaPods；只验证 CocoaPods 也不够，因为 Flutter 3.44+ 的默认路径已经变成 SPM。
...
CocoaPods 常用 `resource_bundles`，SPM 则把资源限定在具体 target 下。普通资源可以用 `resources: [.process("Resources")]` 声明；从 Swift tools 5.3 开始，xib、storyboard、Core Data model、asset catalog 等 Apple 平台常见资源也会被自动处理。
...
最常见的问题不是「有没有写 `resources`」，而是资源放错 target 或运行时取错 bundle。资源必须属于某个 package target；如果你把资源留在 App 工程里，却在 package 代码里按模块资源去取，运行时就会找不到。
...
SPM 引入的源码 package 通常不需要手动处理嵌入方式，但二进制 SDK 要看厂商交付形态：有的是静态 `.xcframework`，有的是动态 `.xcframework`，还有的额外依赖系统库或资源包。
...
如果团队内部有私有 SPM 仓库，CI 上要配置 SSH Key 或 Personal Access Token。SPM 对鉴权链路的报错比较直接，常见原因是：
...
迁移过程本身不算难，难的是开始动那份已经跑了很多年的 `Podfile`。CocoaPods 还能用，但它已经进入维护模式；SPM 是 Swift 生态的官方方案。这事儿早晚要做，趁依赖还没失控时做，心态会好很多。

如果你的项目正卡在迁移第一步——比如某个商业 SDK 没 SPM 支持、或者 CI 缓存搞不定——欢迎留言，我们一起看看怎么解。
