...
这些事情单独看都不大，堆到真实项目里就很烦。

尤其是移动端项目。一个需求可能同时改 Flutter 页面、Swift 原生桥接、路由、权限、构建配置。AI 改代码的速度很快，如果每次 diff 里混着格式化问题、无关空格、缩进变化，后面 review 会非常难受。

所以这篇不再讲 Prompt，也不继续聊 `AGENTS.md`。我们直接做一件小事：用 Claude Code Hooks 配一个最小自动化，让 AI 通过 Edit / Write 改完文件后，自动处理 Dart / Swift 的格式化。

---

## 一、Hooks 到底解决什么问题

可以先把 Hooks 理解成固定时机触发的脚本。

在 Claude Code 里，AI 会不断执行工具，比如读文件、改文件、运行命令。Hooks 就是在这些节点前后插入你自己的命令。
...
这和 Prompt、规则文件、Skills、MCP 不是一类东西。
...
Hooks 的价值就在这里：不用等 AI 想起来，也不用你每次提醒。

---

## 二、移动端项目为什么适合先配 format Hook

移动端项目当然可以做更复杂的自动化，比如构建、测试、打包、发布。但第一个 Hook 不建议这么重。
...
| `Podfile` | 本例不处理，后续可加提醒 | 构建配置改动要谨慎 |
| `pubspec.yaml` | 本例不处理，后续可加提醒 | 涉及依赖和资源声明 |
| `Info.plist` | 本例不处理，后续可加提醒 | 涉及权限、配置、审核风险 |
...
下面以一个 iOS + Flutter 混合项目为例。目标很简单：

- AI 通过 Edit / Write 修改 `.dart` 文件后，自动执行 `dart format`
- AI 通过 Edit / Write 修改 `.swift` 文件后，如果项目存在 `scripts/format-swift.sh`，就调用它
- 其他文件不处理

这个示例脚本用 `jq` 解析 Claude Code 传进来的 JSON。机器上没有 `jq` 的话，先安装它；也可以把解析逻辑换成项目里已有的 Python / Ruby 脚本。
...
这里没有直接写 SwiftFormat 或 swift-format，是有意的。Swift 格式化工具在不同团队里差异比较大，Hook 最好只负责调度，不要偷偷替项目决定工具。
...
Hook 只调用它。项目暂时没有 Swift 格式化脚本也没关系，`.swift` 文件会被跳过，不会阻塞 AI 继续工作。
...
接下来改 Claude Code 的项目配置。在项目根目录创建或编辑：
...
用 `PostToolUse` 是因为 format 应该发生在文件被修改之后。AI 还没写文件之前，format 没有意义。

`Edit|Write` 覆盖最常见的文件修改入口。也可以按自己的使用情况继续扩展，但刚开始不建议把范围放得太大。如果日志里看到 Claude Code 用 `MultiEdit` 批量改文件，再把 matcher 扩成 `Edit|Write|MultiEdit`。
...
正常情况下，会看到刚才配置的 `PostToolUse` Hook。

然后准备一个临时 Dart 文件，让 AI 做一个很小的修改。比如：

```
帮我在这个 Dart 文件里做一个很小的改动，保持原有逻辑不变。
```

AI 修改完成后，看一下 `git diff`。Hook 生效的话，`.dart` 文件已经被格式化，不需要再手动执行：
...
第一个版本只做 format，已经够用了。后面真要扩展，我会先看这几类。
...
更合理的做法是：AI 修改这些文件后，再加一个 Hook 输出提醒，让你在 review 时重点看。前面的示例脚本只做 format，没有包含这段提醒逻辑。

### 2. Stop 时提醒验证结果

另一个实用的 Hook 是 `Stop`。它可以在 AI 准备结束回复时触发，提醒 AI 不要只说“已完成”，而要说明：
...
这和我前面写 Superpowers 时提到的完成前验证是一回事。区别在于：Skill 是流程要求，Hook 是固定时机提醒。

### 3. PreToolUse 拦截危险命令

如果你更关心安全，可以再看 `PreToolUse`。它适合在工具执行前做检查，比如：
...
这些动作一旦发生，影响比较大。比起事后发现，不如事前拦一下。

---

## 八、Hooks 和 Skills 应该怎么配合

前一篇我写过 Superpowers 和 Skills。把它们放在一起看，大概是这样：
...
Skills 更像作业手册，Hooks 更像自动化卡点。两者不是互相替代，而是互相补位。

---

## 九、不要把 Hooks 配得太重

Hooks 很有用，但不适合一上来就配一堆。我见过最容易走偏的是这几类：
...
改完代码自动 format，属于低风险动作。修改证书、签名、依赖、发布配置，属于高风险动作。前者可以自动做，后者更适合提醒和人工确认。

---

## 写在最后

AI Coding 的工程化，不是把所有事情都交给 AI，而是把一些容易忘、但每次都应该做的动作，从聊天提醒里拿出来，放进项目自动化里。

`AGENTS.md` 解决“规则写在哪里”。Skills 解决“任务应该按什么流程做”。Hooks 解决“到了固定时机必须执行什么”。

对 iOS / Flutter 开发者来说，第一个值得配置的 Hook，就是改完代码自动 format。它简单、低风险、收益稳定。先从这里开始，让 AI 不只是会写代码，也能在写完以后顺手把尾收干净。
